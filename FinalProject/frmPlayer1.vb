...
Public Class frmPlayer1
    ...
    Private Sub CardClick(sender As Object, e As EventArgs) Handles picBox1.Click, picBox2.Click, picBox3.Click, picBox4.Click, picBox5.Click, picBox6.Click, picBox7.Click, picBox8.Click, picBox9.Click, picBox10.Click, picBox11.Click, picBox12.Click, picBox13.Click, picBox14.Click, picBox15.Click, picBox16.Click, picBox17.Click, picBox18.Click
        If frmGamemaster.intCurrent = 1 Then
            Dim intIndex As Integer = sender.Name.Substring(6) - 1
            If Not picBoxes(intIndex).Image Is Nothing Then

                If MyCards(intIndex).Compare(frmGamemaster.Discard) Then
                    frmGamemaster.Discard = MyCards(intIndex)
                    frmGamemaster.picDiscard.Image = frmGamemaster.Discard.Cover

                    If MyCards(intIndex).Number <> -1 And MyCards(intIndex).Number <> -2 And MyCards(intIndex).Number <> -3 Then
                        frmGamemaster.lblDiscardNum.Text = MyCards(intIndex).Number
                    Else
                        frmGamemaster.lblDiscardNum.Text = ""
                    End If
                    If MyCards(intIndex).Number = -1 Then
                        MsgBox("Select a color", Title:="Color Prompt.")
                    End If
                    For intIndex = intIndex + 1 To UBound(MyCards) - 1
                        MyCards(intIndex - 1) = MyCards(intIndex)
                    Next intIndex
                    ReDim Preserve MyCards(UBound(MyCards) - 1)

                    Card.Redraw(lblLabels, picBoxes, MyCards)
                Else
                    MsgBox("Invalid Card. Draw a card or play a different card.", Title:="Invalid Card.")
                End If
            End If
        End If
    End Sub
